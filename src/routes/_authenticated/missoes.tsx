...
const LOGIN_MISSION: Mission = {
  ...
  progress: (p) => (p.loggedToday ? 1 : 0), reward: { gems: 5, xp: 5 },
};
...
function getClaimed(): Set<string> {
  ...
  try { const raw = localStorage.getItem(CLAIMED_KEY); const all: string[] = raw ? JSON.parse(raw) : []; return new Set(all.filter((k) => k.endsWith(`:${today()}`))); } catch { return new Set(); }
}
function setClaimed(s: Set<string>) { try { localStorage.setItem(CLAIMED_KEY, JSON.stringify([...s])); } catch {} }
function dayBaselineXp(xp: number) {
  if (typeof window === "undefined") return xp;
  try { const raw = localStorage.getItem(DAY_XP_KEY); const parsed = raw ? JSON.parse(raw) as { date: string; xp: number } : null; if (parsed && parsed.date === today()) return parsed.xp; localStorage.setItem(DAY_XP_KEY, JSON.stringify({ date: today(), xp })); return xp; } catch { return xp; }
}
function MissoesPage() {
  ...
  async function claim(m: Mission) {
    if (!profile) return;
    const key = `${m.id}:${today()}`; const claimed = getClaimed(); if (claimed.has(key)) return;
    const rewards: RewardAmount[] = [
      m.reward.xp ? { type: "xp", amount: m.reward.xp } : null,
      m.reward.gems ? { type: "gems", amount: m.reward.gems } : null,
      m.reward.focus ? { type: "focus", amount: m.reward.focus } : null,
    ].filter((r): r is RewardAmount => Boolean(r));
    // A recompensa só altera os contadores depois que a coleta visual terminar.
    await collectRewards(rewards, document.activeElement instanceof HTMLElement ? document.activeElement : null);
    await updateProfile(profile.id, {
      xp: profile.xp + (m.reward.xp ?? 0),
      gems: profile.gems + (m.reward.gems ?? 0),
      focus: profile.focus + (m.reward.focus ?? 0),
    });
    claimed.add(key); setClaimed(claimed);
  ...
  return (
    <div className="mobile-shell">
      ...
      <main className="flex-1 px-4 py-5 space-y-4">
        ...
        {p && missions.map((m) => {
          const progress = m.progress(p), complete = progress >= m.target, already = claimed.has(`${m.id}:${today()}`);
          return <div key={m.id} className="rounded-2xl bg-card p-4 shadow-card"><div className="flex items-start justify-between gap-3"><div><div className="font-black">{t(m.title)}</div><div className="text-xs text-muted-foreground">{t(m.desc)}</div></div><div className="flex items-center gap-2 text-xs font-bold">{m.reward.xp && <span className="flex items-center gap-1 text-gold"><Trophy className="h-3 w-3" />{m.reward.xp}</span>}{m.reward.gems && <span className="flex items-center gap-1 text-primary"><Gem className="h-3 w-3" />{m.reward.gems}</span>}{m.reward.focus && <span className="flex items-center gap-1 text-yellow-500"><Brain className="h-3 w-3" />{m.reward.focus}</span>}</div></div><div className="mt-3 h-2 rounded-full bg-muted overflow-hidden"><div className="h-full bg-gradient-primary transition-all" style={{ width: `${Math.min(100, (progress / m.target) * 100)}%` }} /></div><div className="mt-2 flex items-center justify-between"><span className="text-xs text-muted-foreground">{progress} / {m.target}</span>{already ? <span className="flex items-center gap-1 text-xs font-bold text-success"><Check className="h-3 w-3" /> {t("Recebido")}</span> : complete ? <button onClick={() => claim(m)} className="btn-3d rounded-full bg-primary px-4 py-1.5 text-xs font-bold text-primary-foreground">{t("Coletar")}</button> : <span className="text-xs text-muted-foreground">{t("Em progresso")}</span>}</div></div>;
        })}
      </main><BottomNav />
    </div>
  );
