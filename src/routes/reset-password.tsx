...
function ResetPassword() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = z.string().min(6, t("Mínimo 6 caracteres")).safeParse(pass);
  ...
  return (
    <div className="mobile-shell px-6 pt-10">
      <div className="flex flex-col items-center text-center">
        ...
        <h1 className="mt-2 text-2xl font-black">{t("Nova senha")}</h1>
        <p className="text-sm text-muted-foreground">{t("Escolha uma senha forte que você lembre.")}</p>
      </div>
      <form onSubmit={onSubmit} className="mt-6 flex flex-col gap-3">
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{t("Nova senha")}</span>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{t("Confirmar")}</span>
        ...
        <button disabled={loading} className="btn-3d mt-2 rounded-2xl bg-primary py-3.5 font-bold text-primary-foreground">
          {loading ? t("Salvando...") : t("Atualizar senha")}
        </button>
      </form>
    </div>
  );
